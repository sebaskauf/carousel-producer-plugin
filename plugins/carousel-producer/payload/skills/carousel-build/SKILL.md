...
in fertige, animierte Carousel-Slides im SKAILE/Claude-Code-Editorial-Stil und rendert sie aus.
...
Warum dieser Skill existiert: Der Look ist hart erarbeitet (Editorial-Serif auf Cream, echte
Claude-Tamagotchis, handgezeichnete Marker, Terminal-Fenster). Ohne festen Stil und feste
Bausteine wuerde jeder neue Post anders und schlechter aussehen. Dieser Skill konserviert genau
das, was funktioniert, und hebt die Qualitaet an den Stellen, die im Prototyp noch schwach waren.
...
- macOS-Fonts: Georgia (Serif), Impact (Display), "Bradley Hand" (Handschrift), JetBrains Mono/Menlo
...
1. **Projekt vorbereiten.** Arbeite in einem HyperFrames-Projekt-Ordner (z.B.
   `~/Downloads/<thema>-carousel/`). Falls keiner existiert: `npx hyperframes init` oder den
   Prototyp `~/Downloads/skaile-carousel/` als Vorlage kopieren (er hat schon `package.json`,
   `hyperframes.json`, `assets/`, `preview-gallery.html`). Lege `design.md` mit der Stil-DNA an.
...
- **Cover = statisch + scroll-stoppend.** Echtes generiertes Bild (Selfie/Szene) full-bleed +
...
- **Echte Claude-Tamagotchis** aus `assets/cut/` (3D-Terracotta, `>‹`-Augen). NIE smooth-SVG-Blobs
  oder Pixel-SVG selbst bauen — das wurde mehrfach verworfen.
...
- Deutsch, echte Umlaute (ä/ö/ü/ß), keine Em-Dashes, Voice = Sebastian (siehe `tone-of-voice`).
...
Wenn Sebastian eine Slide kritisiert: erst den Settle-Frame anschauen, das konkrete Problem benennen
(Abstand? Ausschnitt? Motiv?), gezielt fixen, neu rendern, gegenpruefen. Wiederkehrende Lektionen
gehoeren in `references/stil-dna.md` (nicht hier in die Prozessdatei).
