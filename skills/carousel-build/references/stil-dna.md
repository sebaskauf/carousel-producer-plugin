# Stil-DNA — der Beispiel-Look (Claude-Code-Editorial-Carousel)

Basis-Look fuer Slides. Lies das, bevor du eine Slide baust, und lege die Werte als `design.md` ins
Projekt (HyperFrames liest `design.md` automatisch). **Aber: die `carousel-profil.md` deines Nutzers
schlaegt jede Angabe hier** — hat er eigene Farben, einen eigenen Charakterstil oder dunkle
Grundstimmung gewaehlt, ersetzt du die entsprechenden Werte in der `design.md` durch seine. Nur wer
im Setup "Beispiel-Look uebernehmen" gesagt hat, faehrt 1:1 diese Datei.

## WICHTIG: Look ist Brand, der letzte Post ist NICHT die Vorlage
Diese Stil-DNA beschreibt einen **wiedererkennbaren Look** (Brand). Den haelt jeder Post. Was ein
...
- Charakter-Look als Figuren-Sprache (Beispiel-Look: Terracotta-3D-Maskottchen, `>‹`-Augen, glossy clay — oder der Charakterstil aus dem Profil des Nutzers).
...
- **Pflicht am Composition-Root:** `data-width="1080" data-height="1350"` — ohne diese Attribute rendert HyperFrames im Default-Format (1080x1920) und unten bleibt tote Flaeche.
...
- **Keine Em-Dashes.** Echte Umlaute ä/ö/ü/ß. Voice = dein Nutzer (sein voice-profil).
